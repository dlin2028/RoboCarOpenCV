...
trap_top_width = 100  # ditto for top edge of trapezoid
trap_height = 100  # height of the trapezoid expressed as percentage of image height
...
def draw_lines(img, lines, color=[255, 0, 0], thickness=10):
	...
	# Split lines into right_lines and left_lines, representing the right and left
	# lane lines
	...
	if abs(right_x1) > 1000000:
		draw_right = False
	if abs(left_x1) > 1000000:
		draw_right = False
...
def filter_white(image):
	"""
	Filter the image to include only white pixels
	"""
	# Filter white pixels
...
def filter_orange(image):
    """
	Filter the image to include only orange pixels
	"""
    ...
    orangeRes = cv2.bitwise_and(image,image, mask= mask)
...
def annotate_image_array(image_in):
	...
	# Only keep white and yellow pixels in the image, all other pixels become
	# black
	image = filter_white(image_in)
...
def annotate_center_lines(image_in):
	""" Given an image Numpy array, return the annotated image as a Numpy array """
	# Only keep white and yellow pixels in the image, all other pixels become
	# black
	image = filter_orange(image_in)
	...
	# Run Hough on edge detected image
	line_image = hough_line(masked_edges, rho, theta, threshold, min_line_length, max_line_gap)
